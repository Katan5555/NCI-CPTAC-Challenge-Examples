...
## loading data
RES = pd.read_csv(path + name_of_response, index_col = 0)
features = pd.read_csv(path + name_of_features, index_col = 0) 

nfolds = 10    
num_round = 10

result_prediction = []   ## store result for each protein

for t in range (0, len(RES.iloc[0,:]) ):  
 
    pp_by_protein = []   ## store result for each fold of cross validation
    ...
    result_prediction.append(np.mean(pp_by_protein))   ## append mean over k fold cross validation

result_prediction= pd.DataFrame(result_prediction)     ## np.mean(result_prediction)
...
# iteration=3   ## choose the number of iteration starting from which to average the result
